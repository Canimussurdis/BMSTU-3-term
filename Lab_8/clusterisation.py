...
class Clusterisation:
    ...
    def kmeans(self, plt):
        ...
        plt.scatter(centers[:, 0], centers[:, 1], c="b", s=50)   # Точечный график y против x с различным размером маркера и / или цветом
        plt.title("K-Means, 15")
    ...
    def afprop(self, plt):
        ...
        plt.title("AffinityPropagation, " + str(number))
    ...
    def mean_shift(self, plt):
        ...
        plt.title("Means shift, " + str(n_clusters_))
    ...
    def agglomerative_clustering(self, plt):
        ...
        plt.scatter(self.X[:, 0], self.X[:, 1], marker='.', c=y_pred, cmap='rainbow' )
        plt.title("Agglomerative clustering, " + str(n_clusters))
